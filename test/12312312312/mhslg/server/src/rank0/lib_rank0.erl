...
refresh_rank() ->
	...
	RankerList2 = lists:sublist(RankerList1, mod_rank0:rank_max_number(country_id(), RankType)),
	{_, RankerList3} =
...
filter(_, Ranker) ->
	Ranker#ranker.value > 0.

compare(_, L, R) ->
	try
		?JUDGE_RETURN(L#ranker.value =:= R#ranker.value, L#ranker.value > R#ranker.value),
...
request_rank_list(RoleId) ->
	Rank = get_rank(),
	RankList = Rank#rank.rank_list,
	ProtoRankList = [ranker_to_proto(Ranker) || Ranker <- RankList],

	SelfRanker =
		case is_have_ranker(RoleId) of
			false -> undefined;
			true -> ranker_to_proto(ranker_info(RoleId))
		end,

	Respond = #'Proto50109002'{
		rank_list = ProtoRankList,
		self_rank = SelfRanker,
		country = Rank#rank.country_id,
		rank_type = Rank#rank.type
	},
	lib_send:respond_to_player(RoleId, Respond).

ranker_to_proto(Ranker) ->
	#'ProtoRanker'{
		role_show = lib_common:player_show_to_proto(Ranker#ranker.show),
		hero_base = lib_hero_api:hero_base_to_proto(Ranker#ranker.hero_base),
		max_force_hero = Ranker#ranker.max_force_hero,
		rank = Ranker#ranker.rank,
		value = Ranker#ranker.value,
		update_tick = Ranker#ranker.update_tick
	}.