...
ranker_to_Bson(Ranker) when is_record(Ranker, ranker) ->
	#{
		...
		?BSONSET_INT(max_force_hero, Ranker#ranker.max_force_hero),
		?BSONSET_BSON(show, lib_common:player_show_to_bson(Ranker#ranker.show)),
		?BSONSET_INT(value, Ranker#ranker.value),
		?BSONSET_INT(update_tick, Ranker#ranker.update_tick),
		?BSONSET_BOOL(hero_base, lib_hero_api:hero_base_to_bson(Ranker#ranker.hero_base))
	}
.

load_rank(CountryId, RankType) ->
	case mongo_poolboy:find_one(?MMO_POOL, ?DB_RANK0, #{?BSONSET_INT(country_id, CountryId), ?BSONSET_INT(rank_type, RankType)}) of
		Bson = #{} ->
...
save_rank(Rank) when is_record(Rank, rank) ->
	...
	mongo_poolboy:async_update(?MMO_POOL, ?DB_RANK0, #{
		?BSONSET_INT(country_id, Rank#rank.country_id),
		?BSONSET_INT(rank_type, Rank#rank.type)},
	Bson).
