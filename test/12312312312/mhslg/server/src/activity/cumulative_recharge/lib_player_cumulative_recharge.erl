...
%%%
%%% @end
%%% Created : 09. 六月 2021 11:53
%%%-------------------------------------------------------------------
-module(lib_player_cumulative_recharge).
-author("13661").
...
data_to_bson(Data) ->
    ...
    maps:fold(fun(_, Grade, List) -> 
        BsonGrade = #{
            ?BSONSET_INT(index, Grade#acc_recharge_grade.index),
            ?BSONSET_INT(recharge_number, Grade#acc_recharge_grade.recharge_number),
            ?BSONSET_INT(state, Grade#acc_recharge_grade.state)
        },
        [BsonGrade | List]
    end, [], GradeMap),
...
grade_to_proto(Grade) when is_record(Grade, acc_recharge_grade) ->
    #'ProtoAccRechargeGrade'{
        index = Grade#acc_recharge_grade.index,
        recharge_number = Grade#acc_recharge_grade.recharge_number,
        state = Grade#acc_recharge_grade.state
    }
.
...
tips(UID) ->
    ...
    maps:fold(
        fun(_, R, Acc) ->
            case R#acc_recharge_grade.state of
...
grade_info(UID, Index) ->
    Data = lib_player_activity:get_activity_data(UID),
    GradeMap = Data#cumulatice_recharge.grade_map,
    maps:get(Index, GradeMap, #acc_recharge_grade{index = Index})
.
get_acc(UID) ->
    Data = lib_player_activity:get_activity_data(UID),
...
.
update_grade(UID, Grade) when is_record(Grade, acc_recharge_grade) ->
    Data = lib_player_activity:get_activity_data(UID),
    GradeMap = Data#cumulatice_recharge.grade_map,
    NewGradeMap = maps:put(Grade#acc_recharge_grade.index, Grade, GradeMap),
...
get_recharge_number(UID, Index) ->
    ...
    Grade#acc_recharge_grade.recharge_number
.

set_recharge_number(UID, Index, Number) ->
    Grade = grade_info(UID, Index),
    update_grade(UID, Grade#acc_recharge_grade{recharge_number = Number})
.

get_state(UID, Index) ->
    Grade = grade_info(UID, Index),
    Grade#acc_recharge_grade.state
.

set_state(UID, Index, State) ->
    Grade = grade_info(UID, Index),
    update_grade(UID, Grade#acc_recharge_grade{state = State})
.

is_have_conf(ConfIndex) ->
    tb_cumulative_recharge:get(ConfIndex) =/= false
.
is_have_conf(ConfIndex, Index) ->
    Conf = tb_cumulative_recharge:get(ConfIndex),
...
process_recharge(Activity, Gold) ->
    ...
    try 
        ...
        GradeMap = tb_cumulative_recharge:get(ConfIndex),
        LastTips = tips(UID),
        update_acc(UID, Gold),
...
request_info(Msg) ->
    UID = lib_activity_api:uid(Msg#'Proto10150101'.uid),
