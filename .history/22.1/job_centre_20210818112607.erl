...
-module(job_centre).
-export([start_link/0,add_job/1,work_wanted/0,job_done/1,job_centre/1]).

start_link() ->
    Z=ets:file2tab("C:/Users/Administrator/Desktop/work/22.1/job.date"),
    register(job_centre,spawn(job_centre())),
    register(job,spawn(job(Z))),
    true.
...
job(Z) ->
     case ets:info(Z) of
                undefined ->
                    E=ets:new(job,[duplicate_bag,public]),
