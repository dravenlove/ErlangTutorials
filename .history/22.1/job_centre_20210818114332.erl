...
-module(job_centre).
-export([start_link/0,add_job/0,work_wanted/0,job_done/1,job_centre/1]).
...
job_centre() ->
    job=ets:file2tab("C:/Users/Administrator/Desktop/work/22.1/job.date"),
    receive
        {From,{add_job,_}} ->
            add_job(F),
            From ! {self(),Etsid,add_job(F)};

        {From,{work_wanted,_}} ->
            work_wanted(),
            From ! {self(),Etsid,work_wanted()};

        {From,{job_done,_}} ->   
            job_done(),
            From ! {self(),Etsid,job_done()}
    end.    
